...
void ThreadIRCSeed2(void* parg)
{
    ...
    while (!fShutdown)
    {
        ...
        if (fTestNet) {
            Send(hSocket, "JOIN #SABRTEST2\r");
            Send(hSocket, "WHO #SABRTEST2\r");
        } else {
            // randomly join #SABR00-#SABR05
            // int channel_number = GetRandInt(5);

            // Channel number is always 0 for initial release
            int channel_number = 0;
            Send(hSocket, strprintf("JOIN #SABR%02d\r", channel_number).c_str());
            Send(hSocket, strprintf("WHO #SABR%02d\r", channel_number).c_str());
        }
