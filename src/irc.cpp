...
void ThreadIRCSeed2(void* parg)
{
    ...
    while (!fShutdown)
    {
        ...
        if (fTestNet) {
            Send(hSocket, "JOIN #SFTZTEST2\r");
            Send(hSocket, "WHO #SFTZTEST2\r");
        } else {
            // randomly join #SFTZ00-#SFTZ05
            // int channel_number = GetRandInt(5);

            // Channel number is always 0 for initial release
            int channel_number = 0;
            Send(hSocket, strprintf("JOIN #SFTZ%02d\r", channel_number).c_str());
            Send(hSocket, strprintf("WHO #SFTZ%02d\r", channel_number).c_str());
        }
